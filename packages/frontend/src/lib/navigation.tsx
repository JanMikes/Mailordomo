 ...
 * the opinionated `today` command center, the `digest` morning briefing (Phase 9 / D34 — what needs
 * you today, promises due, what Simona handled, what Claude drafted), the `memory` changelog, the
 * all-projects board (`all-projects`), the classic `three-pane` fallback — the "never trapped" escape
 * hatch (D32) — and the `setup` wizard (project → mailbox → repo → Claude health) plus raw-config view
 * (Phase 8 / D33).
 */
export type AppView = 'today' | 'digest' | 'memory' | 'all-projects' | 'three-pane' | 'setup';
